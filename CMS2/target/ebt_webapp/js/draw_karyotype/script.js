...
function parsingData(data){
    ...
    for(var key in newLengthData){
        buttonList[key.replace(/[0-9]/g,"")] = buttonList[key.replace(/[0-9]/g,"")] ? buttonList[key.replace(/[0-9]/g,"")] : {};
    }
...
function drawChart(kindEl){
    ...
    for(var i = 0 ; i < posArr.length ; i++){
        ...
        chartName.innerHTML = keyArr[i] + " <br /> (" + insertComma(posArr[i]) + "bp)";
        ...
        if(searchValue.length){
            for(var j = 0 ; j < searchValue.length ; j++){
                ...
                if(data["chr"] == keyArr[i]){
                    ...
                    chartStackName.innerHTML =searchValue[j]+"<br />"+"("+insertComma(Number(posArr[i]))+")";
...
function drawChartName(kindEl){
    ...
    for(var i = 0 ; i < newArr.length ; i++){
        ...
        chartName.innerHTML = newArr[i].name + " <br /> (" + insertComma(Number(newArr[i].pos)) + "bp)";
...
function drawStandard(highPos){
    ...
    for(var i = 0 ; i < standardEls.length ; i++){
        ...
        number = number * pow;
        standardEls[i].innerText = insertComma(number);
    }
}
// 이름 마진값, 선 길이/각도 계산
            ...
            if(e.path[0].classList.contains("chartEl")){
                ...
                palette.addEventListener("click", function(item){
                    if(item.target.classList.contains("sp-thumb-inner")){
                        ...
                        console.log( item.target.parentElement.dataset.color );
                        console.log( item.target.parentElement.dataset.color );
